...
void Processor::GetCpuValues(std::vector<std::vector<long>> &values) {
  ...
  for (std::string line : lines) {
    ...
    linestream >> name;
    long ticks;
    while (linestream >> ticks) {
      values_one_line.push_back(ticks);
    }
...
std::vector<float> Processor::Utilization() {
  ...
  for (unsigned int i = 0; i < values.size(); i++) {
    ...
    long NonIdle = values[i][0] + values[i][1] + values[i][2] + values[i][5] +
                   values[i][6] + values[i][7];
    long PrevNonIdle = prev_values[i][0] + prev_values[i][1] +
                       prev_values[i][2] + prev_values[i][5] +
                       prev_values[i][6] + prev_values[i][7];
